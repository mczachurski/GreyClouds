...
public protocol SwipeMenuViewDataSource: class {
    ...
    /// Return custom views to be displayed at the specified tag in `SwipeMenuView`.
...
extension SwipeMenuViewDataSource {
    public func swipeMenuView(_ swipeMenuView: SwipeMenuView, viewForPageAt index: Int, withFrame frame: CGRect) -> TabItemView? { return nil }
}
...
extension SwipeMenuView: TabViewDataSource {
    ...
    public func tabView(_ tabView: TabView, viewForItemAt index: Int, withFrame frame: CGRect) -> TabItemView? {
        return dataSource?.swipeMenuView(self, viewForPageAt: index, withFrame: frame)
    }
}
