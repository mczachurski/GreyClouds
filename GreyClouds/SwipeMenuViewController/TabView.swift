...
public protocol TabViewDataSource {
    ...
    func tabView(_ tabView: TabView, viewForItemAt index: Int, withFrame frame: CGRect) -> TabItemView?
}
...
open class TabView: UIScrollView {
    ...
    fileprivate func setupTabItemViews() {
        ...
        for index in 0..<itemCount {
            ...
            let frame = CGRect(x: xPosition, y: 0, width: options.itemView.width, height: containerView.frame.size.height)
            var tabItemView:TabItemView! = dataSource.tabView(self, viewForItemAt: index, withFrame: frame)
            if tabItemView == nil {
                tabItemView = TabItemView(frame: frame)
            }
